...
export function ElasticSlider({
  ...
  /**
   * Controlled `value` / `percentage` from parent (e.g. quick pills): animate fill like a track click.
   * While `isInteracting` (pointer down/drag), pointer handlers own `fillPercent` — do not fight them.
   * useLayoutEffect: start spring before paint to avoid one frame of wrong fill after click.
   */
  useLayoutEffect(() => {
    ...
    if (Math.abs(fillPercent.get() - percentage) < 0.01) {
      return
    }

    animateFillTo(percentage)
  }, [percentage, isInteracting, shouldReduceMotion, fillPercent, animateFillTo])
  ...
  return (
    <div
      ...
      className={cn(
        "[--elastic-slider-height:2.25rem] [--elastic-slider-radius:var(--radius-lg)]",
        "[--elastic-slider-bg:var(--muted)]",
        "[--elastic-slider-fill:color-mix(in_srgb,var(--muted-foreground)_10%,transparent)]",
        "[--elastic-slider-fill-active:color-mix(in_srgb,var(--muted-foreground)_20%,transparent)]",
        "[--elastic-slider-hash:color-mix(in_srgb,var(--muted-foreground)_30%,transparent)]",
    ...
    >
      <motion.div
        ...
        className={cn(
          "group/elastic-slider absolute inset-0 cursor-pointer touch-none overflow-hidden rounded-[var(--elastic-slider-radius)] bg-[var(--elastic-slider-bg)] outline-none select-none",
      ...
      >
          ...
          {Array.from({ length: hashMarkCount }, (_, i) => (
            <div
              ...
              className={cn(
                ...
                "bg-transparent group-data-[active=true]/elastic-slider:bg-[var(--elastic-slider-hash)]"
              )}
        ...
        <motion.div
          ...
          className={cn(
            ...
            "bg-[var(--elastic-slider-fill)] group-data-[active=true]/elastic-slider:bg-[var(--elastic-slider-fill-active)]"
          )}
        ...
        <span
          ...
          aria-hidden="true"
          className="pointer-events-none absolute top-1/2 left-3 inline-flex -translate-y-1/2 items-center text-sm leading-none font-medium text-[var(--elastic-slider-label)] transition-colors"
        >
          {label}
        </span>
        ...
        <span
          ...
          aria-hidden="true"
          className={cn(
            "pointer-events-none absolute top-1/2 right-3 -translate-y-1/2 font-mono text-sm leading-none font-medium transition-colors",
            "text-[var(--elastic-slider-label)] group-data-[active=true]/elastic-slider:text-[var(--elastic-slider-focus)]"
          )}
        >
          {displayValue}
        </span>
